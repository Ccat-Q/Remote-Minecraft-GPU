...
after the local vtest trace has selected the no-FD profile. The target is
`src/gallium/winsys/virgl/vtest/virgl_vtest_winsys.c`, specifically
`virgl_vtest_flush_frontbuffer()`.

The patch must replace only the presentation-only call path:
...
`byte_offset` is mandatory. It is maintained by the vtest socket write helper
